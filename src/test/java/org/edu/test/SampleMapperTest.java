...
public class SampleMapperTest {
	...
	* 프로젝트폴더경로 DbInterface/src/main/webapp/WEB-INF/spring/root-context.xml (Mysql서버접속정보 변경해야 합니다.)
	...
	public void testInsertMember() {
		...
		testSelectMember();
		System.out.println("입력 전 리스트------------------------");
		MemberVO vo = new MemberVO();
		vo.setUserid("user" + today);
		vo.setUserpw("1234");
		vo.setUsername("스타벅스");
		vo.setEmail("user999@test.com");
		mapper.insertMember(vo);
		System.out.println("입력 후 리스트------------------------");
		testSelectMember();
	}
	@Test
	...
	public void testUpdateMember () {
		...
		MemberVO vo = new MemberVO();
		//수정은 여러개의 변수값을 변경하기 때문에 MemberVO클래스 변수를 매개변수로 함 
		vo.setUserid("user777");
		vo.setUserpw("9026");
		vo.setUsername("카트라이더");
		vo.setEmail("kartrider@nexon.com");
	...
	public void testDeleteMember() {
		testSelectMember();
		mapper.deleteMember("user888");
		System.out.println("지운 후 회원정보--------------------------");
		testSelectMember();
	}
